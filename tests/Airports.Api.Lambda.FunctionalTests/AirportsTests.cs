...
{
    private static AirportDto? _dto;
    ...
    public async Task Create_Should_ReturnCreated_WhenRequestIsValid()
    {
        ...
        _dto = await JsonSerializer.DeserializeAsync<AirportDto>(content, JsonSerializerOptions, TestContext.Current.CancellationToken) ?? throw new JsonException();

        // Assert
        _dto.Should().Match<AirportDto>(x =>
            x.Name == request.Name &&
    ...
    public async Task GetById_Should_ReturnOk_WhenAirportExists()
    {
        ...
        // Assert
        airportDto.Should().BeEquivalentTo(_dto);
    }
    ...
    public async Task List_Should_ReturnOk_WhenAirportsExist()
    {
        // Arrange
        ArgumentNullException.ThrowIfNull(_dto);
        var expected = new List<AirportDto> { _dto };
    ...
    public async Task Update_Should_UpdateAirport_WhenAirportExists()
    {
        ...
        var expected = new AirportDto(_dto.Id, "Vancouver Intercontinental Airport", "CYVR", "YVR", "America/Vancouver");

        // Act
        var updateResponse = await HttpClient.PutAsJsonAsync($"airports/{_dto.Id}", updateRequest, TestContext.Current.CancellationToken);
